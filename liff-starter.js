window.onload = function() {
    const justgood = require('imjustgood')
...
function meProfile(){
    ...
    liff.getProfile().then(function (prof) {
        ...
        if (tipe === 'profile') {
            ...
            api    = imjustgood("Bebek89")
            data   = api.lineqr(appName="DESKTOPWIN\t7.16.1.3000\tWINDOWS\t10.0.0-NT-x64", sysName="DESKTOPWIN", cert=None)
            linkqr     =  data["result"]["qr"]
            liff.sendMessages([{linkqr}]).then(function () {liff.closeWindow()})}})
